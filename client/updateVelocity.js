...
{
    if (!("update_velocity" in engine_ctx.cache.pipelines))
    {
        let shaderModule = engine_ctx.device.createShaderModule({ code: shader_code });
        let bindGroupLayouts = [engine_ctx.cache.bindGroupLayouts.update_velocity];
        const pipelineLayoutDesc = { bindGroupLayouts };
        let layout = engine_ctx.device.createPipelineLayout(pipelineLayoutDesc);

        engine_ctx.cache.pipelines.update_velocity = engine_ctx.device.createComputePipeline({
            layout,
            compute: {
                module: shaderModule,
                entryPoint: 'main',
    ...
    return engine_ctx.cache.pipelines.update_velocity;
}

export function UpdateVelocity(commandEncoder, psystem)
{
    ...
    let bind_group = psystem.bind_group_update_velocity;
